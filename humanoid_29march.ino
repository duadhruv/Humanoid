...
double servocallibration()
{ 
  serialread(inipos);
  double changeperdegree = 0;
  for(int i=5;i<8;i++)
{ 
  pwm1.setPWM(i, 0, pulseWidth(ang[i]+5));
  int p=analogRead(i);
  pwm1.setPWM(i, 0, pulseWidth(ang[i]-5));
  int q=analogRead(i);
  changeperdegree+= (p-q)/10;
  pwm1.setPWM(i, 0, pulseWidth(ang[i]));
}
changeperdegree /= servonum;
return changeperdegree;
}
